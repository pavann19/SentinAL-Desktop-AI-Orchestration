...
1. **Hybrid Intent Routing Architecture:** A multi-layered intent classification system using local semantic embeddings (cosine similarity across 15 intents, each seeded with 20+ anchors) before falling back to computationally expensive LLMs. This drastically reduces end-to-end latency and API costs for standard operational commands, achieving a 57.39% deterministic fast-path hit rate across 704 evaluation utterances (see Section 7.1).
...
**Addressing RQ1 (Latency and Accuracy):** The hybrid intent routing architecture—combining deterministic keyword matching, local semantic embeddings via `all-MiniLM-L6-v2`, and LLM fallback—successfully reduces median pipeline latency to 101.5ms across 253 traced invocations. The embedding fast-path resolves 57.39% of 704 evaluation utterances in under 50ms with zero cloud API dependency, while the LLM fallback maintains full intent coverage for the remaining 42.61% of complex queries. Critically, the validation pipeline adds only 0.06ms of overhead (p50), demonstrating that deterministic security checks impose no meaningful performance penalty. This represents orders-of-magnitude improvement over LLM-only architectures while maintaining equivalent functional coverage.
