...
**Core principle:** *The agent that writes the code never certifies it.* Verification is done by a different party (Claude-as-integrator, or a second agent) using evidence the implementing agent could not fabricate.
...
- Tests are written by a **different agent than the implementer** (or by Claude), against the *spec*, before or without seeing the implementation.
...
| Gate | Run by | Cost |
|---|---|---|
| 1 Diff-real | Claude (integrator) — reads diff | cheap |
| 2 Independent tests | test-author agent writes; Claude runs | cheap to run |
| 3 Coverage | scripted (`pytest --cov`) | free |
| 4 Runtime artifact | Claude drives + saves evidence | medium |
| 5 Regression | scripted (full pytest) | free |

**Rule:** Gates 3 and 5 are scripted so they cost almost no agent tokens. Gates 1 and 4 are where Claude spends tokens — keep them focused per-task.
...
- MERGE_LOG.md / task board updated, git committed, tagged (`git tag phase-1-complete`).
